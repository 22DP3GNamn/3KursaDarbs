export default {
    ...
    methods: {
        ...
        if (this.party && this.party.id && window.Echo) {
          window.Echo.channel('party.' + this.party.id)
            .listen('PartyJoined', (e) => {
              this.showNotification(e.message); // e.message is always present for PartyJoined
              this.fetchParty();
            })
            .listen('PartyKicked', (e) => {
      ...
      showNotification(message) {
        // Prevent duplicate notifications with the same message within a short time
        const id = Date.now();
        // Only add if not already present in the last 5 seconds
